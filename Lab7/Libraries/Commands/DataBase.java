...
public class DataBase {
    ...
    public CopyOnWriteArrayList<SpaceMarine> selectAllNotes() throws SQLException, NotDatabaseUpdateException {
        PreparedStatement preparedStatement = connection.prepareStatement("select * from spacemarines ");

        if (preparedStatement.execute()) {

            ResultSet rs = preparedStatement.getResultSet();
            CopyOnWriteArrayList<SpaceMarine> spaceMarines = new CopyOnWriteArrayList<>();
            while (rs.next()) {
                ...
                try {
                    ...
                    throw new NotDatabaseUpdateException("Ошибка в обновлении коллекции");
