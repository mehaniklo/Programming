...
public class ServerPart {
    ...
    private void updateTicketFields(Ticket ticket, String userName) throws SQLException {
        ...
        do {
            waitForWrite(response + "Enter the coordinates: (в формате x y)", userName);
            response = "";
            ...
            try {
                ...
                else response += "Enter the correct x and y values (они должны быть больше -48) \n";
            } catch (NumberFormatException e) {response += "Enter the correct x and y values (они должны быть больше -48) \n";}
        ...
        if (ticket.getId() != 0 || ticket.getPrice() == null) {
            ticket.setPrice(-1.0);
            do {
                waitForWrite(response + "Enter the ticket price: (она должна быть больше 0)", userName);
                response = "";
                ...
                try {
                    ...
                    response += "Enter the correct priceь \n";
                }
            } while (ticket.getPrice() <= 0);
        }
        do {
            waitForWrite(response + "Enter the ticket type:  \n VIP, USUAL, BUDGETARY, CHEAP", userName);
            response = "";
            s = (String) waitForRead(userName, "");
            if (!s.equals("")) {
                try {
        ...
        waitForWrite("Where is the ticket for? ", userName);
        s = (String) waitForRead(userName, "");
        ...
        if (!s.equals("")) {
            ...
            s = "";
            do {
                waitForWrite(response + "Enter the audience capacity:", userName);
                response = "";
                try {
                    s = (String) waitForRead(userName, "");
            ...
            do {
                waitForWrite(response + "Enter the audience type:  \n   BAR, LOFT, THEATRE, MALL, STADIUM", userName);
                response = "";
                s = (String) waitForRead(userName, "");
                if (!s.equals("")) {
                    try {
